...
The stochastic differential equations are highly effective tools for financial market forecasting. They can be brought into the PBD to create a novel approach for structural design:
...
The problem with the above model is that the FX market is a decentralized one for which the order flows are generally _not_ available. Therefore $B(t)$ and $S(t)$ forces can _not_ be accessed reliably. So, this model won't work for FX market.
